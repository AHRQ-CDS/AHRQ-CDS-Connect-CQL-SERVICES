  ...
  for (const fileName of fs.readdirSync(pathToFolder)) {
    ...
    if (lib && lib.id) {
      ...
      if (!elm) {
        ...
        if (json.prefetch) {
          console.log(`WARNING: Overriding default prefetch for ${json.id} with user-configured ` +
            'prefetch.  Please ensure that the configured prefetch covers ALL resources needed ' +
            'by the CQL, otherwise results may be incomplete or incorrect.');
          // Do some basic error checking to find any resources in the CQL but not in prefetch
          try {
            const extractedPrefetch = extractPrefetchFromELM(elm);
            Object.keys(extractedPrefetch).forEach(resourceName => {
              const covered = Object.keys(json.prefetch).some(key => {
                return json.prefetch[key].startsWith(`${resourceName}?`)
                  || json.prefetch[key].startsWith(`${resourceName}/`);
              });
              if (!covered) {
                console.error(`WARNING: Configured prefetch for ${json.id} does not contain any ` +
                  `queries for ${resourceName}, but CQL library attempts to retrieve it.  ` +
                  `Consider adding:\n  "${resourceName}": "${extractedPrefetch[resourceName]}"`);
              }
            });
          } catch (e) {
            console.error('WARNING: Could not compare configured prefetch with extracted prefetch due to ' +
              'error extracting prefetch from ELM', e);
          }
        } else {
