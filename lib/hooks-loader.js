...
function extractPrefetchFromExpression(prefetch, expression, libraries=[], currentLibraryName='') {
  if (expression && Array.isArray(expression)) {
    expression.forEach(e => extractPrefetchFromExpression(prefetch, e, libraries));
    ...
    if (expression.type === 'Retrieve') {
      ...
      if (expression.libraryName && Object.keys(libraries).includes(expression.libraryName)) {
        ...
        let refdExp = libraries[expression.libraryName].source.library.statements.def.filter( exp => {
        ...
        if (refdExp) {
          extractPrefetchFromExpression(prefetch, refdExp[0], libraries, expression.libraryName);
        }
        ...
        let refdExp = libraries[currentLibraryName].source.library.statements.def.filter( exp => {
          return exp.name == expression.name;
        });
        if (refdExp) {
          // Still need to pass in the library name since we still could be passing in another reference
          extractPrefetchFromExpression(prefetch, refdExp[0], libraries, currentLibraryName);
        }
      }
      ...
      for (const val of Object.values(expression)) {
        extractPrefetchFromExpression(prefetch, val, libraries);
      }
    }
  }
}
