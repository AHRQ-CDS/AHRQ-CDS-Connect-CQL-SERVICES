...
function extractPrefetchFromELM(elm) {
  ...
  if (elm && elm.source && elm.source.library && elm.source.library.statements && elm.source.library.statements.def) {
    for (const expDef of Object.values(elm.source.library.statements.def)) {
      // Need to pass in entire elm object in case we need to follow references
      extractPrefetchFromExpression(prefetch, expDef.expression, elm);
    }
  }
  return prefetch;
}

function extractPrefetchFromExpression(prefetch, expression, elm=[], currentLibraryName='') {
  if (expression && Array.isArray(expression)) {
    expression.forEach(e => extractPrefetchFromExpression(prefetch, e, elm));
    ...
    if (expression.type === 'Retrieve') {
      ...
      if (expression.libraryName && Object.keys(elm.includes).includes(expression.libraryName)) {
        // Try to find the expression being referenced in the appropriate library
        let refdExp = elm.includes[expression.libraryName].source.library.statements.def.filter( exp => {
        ...
        if (refdExp) {
          extractPrefetchFromExpression(prefetch, refdExp[0], elm, expression.libraryName);
        }
        ...
        let refdExp = elm.includes[currentLibraryName].source.library.statements.def.filter( exp => {
          return exp.name == expression.name;
        });
        if (refdExp) {
          // Don't need to pass the library name in here since we're not passing in an expression reference.
          extractPrefetchFromExpression(prefetch, refdExp[0], elm);
        }
      }
      ...
      for (const val of Object.values(expression)) {
        extractPrefetchFromExpression(prefetch, val, elm);
      }
    }
  }
}
