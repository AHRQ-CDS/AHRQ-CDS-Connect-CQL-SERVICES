...
function extractPrefetchFromELM(elm) {
  ...
  if (elm && elm.source && elm.source.library && elm.source.library.statements && elm.source.library.statements.def) {
    ...
    // Check included libraries for any more prefetch requirements
    if (elm.includes) {
      for (const lib of Object.values(elm.includes)) {
        if (lib.source && lib.source.library && lib.source.library.statements && lib.source.library.statements.def) {
          for (const libExpDef of Object.values(lib.source.library.statements.def)) {
            extractPrefetchFromExpression(prefetch, libExpDef.expression);
          }
        }
      }
    }
